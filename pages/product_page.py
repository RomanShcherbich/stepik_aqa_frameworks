...
class ProductPage(BasePage):
    ...
    def load_goods_data(self):
        self.name = self.browser.find_element(*ProductPageLocators.GOOD_NAME).text.strip()
        self.price = self.browser.find_element(*ProductPageLocators.GOOD_PRICE).text.strip()
        print(f"\nname: {self.name}")
    ...
    def validate_good_added_to_basket(self):
        basket_messages = self.browser.find_elements(*ProductPageLocators.SUCCESS_MESSAGE)
    ...
    def should_not_be_success_message(self):
        assert self.is_element_present(*ProductPageLocators.SUCCESS_MESSAGE, implicitly_timeout=4), \
